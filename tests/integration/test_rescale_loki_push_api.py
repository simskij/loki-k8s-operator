...
import pytest
import yaml
from helpers import is_loki_up, juju_show_unit, oci_image
...
async def test_scale_up_to_three_units(ops_test: OpsTest):
    ...
    assert all(
        await asyncio.gather(
            is_loki_up(ops_test, app_name, unit_num=0),
            is_loki_up(ops_test, app_name, unit_num=1),
            is_loki_up(ops_test, app_name, unit_num=2),
        )
    )
